...
async function getRedirectLinks(req, res) {
  try {
    ...
    if (end) {
      ...
      const loi = Math.floor((Date.now() - new Date(data.start_date_time).getTime()) / 60000); // Calculate LOI in minutes

      const query2 = `
          UPDATE project_report
          SET end_date_time = NOW(), status = ?, loi = ?
          WHERE hash_identifier = ?`;
      ...
      await pool.query(query2, [statusMap[end].status, uid, loi]);
  ...
  try {
    ...
    if (isTestLink === 0) {
      ...
      if (await checkDuplicateSupplierUser(originalUid, stid)) {
        ...
        supplierCPI = projectDetails.supplier_cpi;
    ...
    let params = {
      linkStatus : status,
      projectCPI: projectCPI,
      supplierCPI: supplierCPI,
    ...
    await logProjectReport(params); // Pass the final parameters
...
async function logProjectReport(p) {
  ...
  const hashString = normalizeHash(p.hashIdentifier);
